...
--
--  v2 (2026): добавлены:
--    • Workload.Подгруппа  — деление класса на подгруппы (NULL=весь класс, 1/2=подгруппа)
--    • Schedule.Чётность_недели — поддержка предметов 1 раз в 2 недели (0=каждую, 1=нечётные, 2=чётные)
--    • Убран UNIQUE на кабинет в Schedule — логика перенесена в триггер с учётом чётности недели
--
--  Особенности SQLite, учтённые при переносе с SQL Server:
...
--    • хранимые процедуры    -> класс StoredProcedures.cs
--  Включение внешних ключей обязательно при каждом подключении:
...
-- Нагрузка (учитель + класс + предмет + часы в неделю + подгруппа)
-- Подгруппа: NULL = весь класс, 1 = первая подгруппа, 2 = вторая подгруппа
CREATE TABLE Workload (
    ...
    Подгруппа                 INTEGER DEFAULT NULL CHECK (Подгруппа IS NULL OR Подгруппа IN (1, 2))
);
...
-- Расписание (нагрузка + кабинет + день + урок + чётность недели)
-- Чётность_недели: 0=каждую неделю, 1=нечётные недели, 2=чётные недели
-- Уникальность кабинета контролируется триггером trg_PreventDoubleBooking
-- с учётом перекрытия чётностей (нельзя поставить в один кабинет "каждую нед." и "нечётные" —
-- они пересекаются; но "нечётные" и "чётные" в одном кабинете — разрешено).
...
-- Триггер 1: запрет двойного бронирования кабинета с учётом чётности недели.
-- Конфликт есть, если чётности перекрываются:
--   0 (каждую нед.) конфликтует с 0, 1 и 2 — он присутствует на всех неделях.
--   1 (нечётные) конфликтует с 0 и 1, но НЕ с 2.
--   2 (чётные)   конфликтует с 0 и 2, но НЕ с 1.
...
-- Полное расписание — основной вид, включает чётность недели и подгруппу
...
SELECT
    ...
    diff.Сложность,
    s.Чётность_недели,
    CASE s.Чётность_недели
        WHEN 0 THEN ''
        WHEN 1 THEN '[Нч]'
        ELSE '[Чт]'
    END AS Пометка_недели,
    w.Подгруппа,
    CASE
        WHEN w.Подгруппа IS NULL THEN ''
        WHEN w.Подгруппа = 1    THEN '[П1]'
        ELSE '[П2]'
    END AS Пометка_подгруппы
FROM Schedule s
...
-- Нагрузка с количеством поставленных уроков и типом (весь класс / подгруппа)
...
SELECT
    ...
    w.Подгруппа,
    CASE
        WHEN w.Подгруппа IS NULL THEN 'Весь класс'
...
-- Конфликт = одинаковый учитель ИЛИ один кабинет в одно время с перекрывающейся чётностью.
-- Чётности не перекрываются только у пары (1, 2) — нечётные и чётные не пересекаются.
